...
const updateBook = asyncHandler(async (req, res) => {
    ...
    // Capture old stock & price to check for restock & price drop
    const oldStock = book.stock || 0;
    const newStock = req.body.stock !== undefined ? Number(req.body.stock) : oldStock;

    const oldEffectivePrice = book.discountPrice || book.price || 0;
    ...
    const newEffectivePrice = book.discountPrice || book.price || 0;

    // Logic for Notifications (Restock, Price Drop & Low Stock)
    try {
        const isRestocked   = oldStock === 0 && newStock > 0;
        const isPriceDropped = newEffectivePrice < oldEffectivePrice;

        // ── Wishlist Users: Restock & Price Drop ──────────────────
        if (isRestocked || isPriceDropped) {
            const usersWithBook = await User.find({ wishlist: book._id });

            if (usersWithBook.length > 0) {
                const notifications = [];
                usersWithBook.forEach(u => {
                    if (isRestocked) {
                        notifications.push({
                            user:    u._id,
                            message: `Good news! The book "${book.title}" from your wishlist is back in stock.`,
                            type:    "system"
                        });
                    }
                    if (isPriceDropped) {
                        notifications.push({
                            user:    u._id,
                            message: `Price Drop Alert! The price of "${book.title}" in your wishlist has dropped to Rs. ${newEffectivePrice}.`,
                            type:    "promo"
                        });
                    }
                });
        ...
        // Trigger when stock is updated and final stock is 5 or less (but not 0, which is "Out of Stock")
        const stockWasChanged = req.body.stock !== undefined && newStock !== oldStock;
        if (stockWasChanged && newStock >= 0 && newStock <= 5) {
            const admins = await User.find({ role: "admin" }).select("_id");
            if (admins.length > 0) {
                const stockLabel = newStock === 0 ? "out of stock" : `only ${newStock} left`;
                const adminNotifs = admins.map(admin => ({
                    user:    admin._id,
                    message: `⚠️ Low Stock Alert: "${book.title}" is ${stockLabel} in stock.`,
                    type:    "system"
                }));
                await Notification.insertMany(adminNotifs);
                console.log(`✅ Low stock notification sent for "${book.title}" (stock: ${newStock})`);
            }
        }
...
const getBestsellerBooks = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, books, "Bestseller books fetched successfully"))
})
...
export {
    ...
    getFeaturedBooks, getBestsellerBooks, getDealsBooks,
